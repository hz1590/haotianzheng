...
author: "Haotian Zheng, Rongqi Zhang, Keith Lee"
date: "2022-12-14"
output:
...
```
We can learn that an obviously linear trend among cement, ca to strength. Thus, trying to fit a linear model is reasonable.
...
```
P value is large, so we cannot reject the null hypothesis that Fine Aggregate = 0 
...
anova(woca, full)
```
P value is large, so we cannot reject the null hypothesis that Coarse Aggregate = 0 
...
anova(wocanfaa, full)
```
P value is large, so we so we cannot reject the null hypothesis that Coarse Aggregate and Fine Aggregate = 0, which matched with our BIC mode.
